...
$sourceDll = Join-Path $projectDir "bin\$Configuration\netstandard2.1\DKCTileStreamTracer.dll"
$pluginDir = Join-Path $GameDir 'BepInEx\plugins\DKCTileStreamTracer'

if (-not (Test-Path -LiteralPath $gameExe)) { throw "SUPERZSNES.exe was not found in $GameDir" }
if (-not (Test-Path -LiteralPath $bepInExDll)) { throw 'Install BepInEx 5 x86 in the game directory first.' }
if (-not (Test-Path -LiteralPath $sourceDll)) { throw "Build the project first; DLL not found at $sourceDll" }

New-Item -ItemType Directory -Path $pluginDir -Force | Out-Null
Copy-Item -LiteralPath $sourceDll -Destination (Join-Path $pluginDir 'DKCTileStreamTracer.dll') -Force
