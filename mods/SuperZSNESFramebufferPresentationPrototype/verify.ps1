...
$project = Join-Path $root 'SuperZSNESFramebufferPresentationPrototype.csproj'
$tests = Join-Path $root 'Tests\SuperZSNESFramebufferPresentationPrototype.Tests.csproj'
$game = (Join-Path $env:SUPERZSNES_MANAGED_DIR 'Assembly-CSharp.dll')
$dll = Join-Path $root 'bin\Release\net472\SuperZSNESFramebufferPresentationPrototype.dll'
...
$installed = @(Get-ChildItem -Recurse -Filter 'SuperZSNESFramebufferPresentationPrototype.dll' -LiteralPath (Join-Path $env:SUPERZSNES_ROOT 'BepInEx\plugins') -ErrorAction SilentlyContinue)
[pscustomobject]@{
    Verified = $true
