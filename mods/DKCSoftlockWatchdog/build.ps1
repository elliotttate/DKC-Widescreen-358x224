...
if (-not (Test-Path -LiteralPath (Join-Path $BepInExRoot 'BepInEx\core\BepInEx.dll') -PathType Leaf)) {
    throw "BepInExRoot does not contain BepInEx\core\BepInEx.dll: $BepInExRoot"
}
if (-not (Test-Path -LiteralPath (Join-Path $GameManagedDir 'UnityEngine.dll') -PathType Leaf)) {
    throw "GameManagedDir does not contain UnityEngine.dll: $GameManagedDir"
}

dotnet build (Join-Path $projectDir 'DKCSoftlockWatchdog.csproj') -c $Configuration `
    "-p:BepInExRoot=$BepInExRoot" "-p:GameManagedDir=$GameManagedDir"
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed with exit code $LASTEXITCODE" }

$previousCleanRom = $env:DKC_CLEAN_ROM
try {
    if ($CleanRomPath) {
    ...
    dotnet run --project (Join-Path $projectDir 'Tests\DKCSoftlockWatchdog.Tests.csproj') -c $Configuration
    if ($LASTEXITCODE -ne 0) { throw "offline tests failed with exit code $LASTEXITCODE" }
}
finally {
    $env:DKC_CLEAN_ROM = $previousCleanRom
}

Write-Host 'Built and verified DKCSoftlockWatchdog.dll. Nothing was installed or launched.' -ForegroundColor Green
