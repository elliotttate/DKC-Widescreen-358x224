...
{
    internal static class DKCLevelAutomationPlugin
    {
    ...
    internal static class BridgeHandleLeakProgram
    {
        private static int Main()
        {
            ...
            {
                Console.WriteLine("Bridge handle test skipped: Windows required.");
                return 0;
            }
            var directory = Path.Combine(Path.GetTempPath(), "dkc-bridge-handle-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(directory);
            var bridge = new LoopbackBridge(Path.Combine(directory, "bridge.json"), new BepInEx.Logging.ManualLogSource(), 5);
            var pumping = true;
            Thread pump = null;
            try
            {
                ...
                Console.WriteLine("Bridge bounded handle test: requests=" + requests + ", before=" + before + ", after=" + after + ", delta=" + delta + ".");
                if (delta > 8) throw new InvalidOperationException("Bridge leaked more than 8 handles across the bounded request loop.");
                return 0;
            }
            finally
            {
        ...
        private static void Request(int port, string token, string id)
        {
            using (var client = new TcpClient())
            {
                ...
                {
                    writer.WriteLine(id + "\t" + token + "\tstatus");
                    var response = reader.ReadLine();
                    if (response == null || response.IndexOf("\"ok\":true", StringComparison.Ordinal) < 0)
                        throw new InvalidOperationException("Bridge request failed: " + response);
                }
            }
        }
